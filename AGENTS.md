...
- **재현/빌드**: `./build.sh [VIEW_DAYS]` (기본 30) → 전체 스캔 → 누적 DB 갱신 → 대시보드 빌드 → 브라우저 자동 오픈.
  - 출력 위치: `~/.cc-usage/` (env `CC_USAGE_HOME` 으로 변경). `history.csv`(누적 DB) + `dashboard.html`(데이터 인라인 self-contained).
  - 테스트 시 `CC_USAGE_OPEN=0` 으로 브라우저 오픈 스킵.
...
- **토큰·비용**(`DATA.tokens`, `tokens.csv`): 줄별 `message.usage`(input/output/cache_read/cache_creation의 5m·1h)를 `(date,by,project,model)` 로 집계. 모델은 정규화(`claude-opus-4-8`→`opus-4-8` 등). 비용 = 토큰 × 리스트가(입력/출력 + 캐시읽기 0.1× / 캐시쓰기 5m 1.25× · 1h 2×). **가격표는 `build.sh` 의 `PRICE` dict에 박혀있음(2026-06 기준) — Anthropic이 바꾸면 거기 한 줄 고친다**(ponytail 캘리브레이션 노브). 미등록 모델(`<synthetic>` 등)은 비용 0, 토큰은 그대로 표시. 비용은 클라에서 `costOf()` 로 필터창마다 재계산(파이썬 echo와 일치 검증함). Spend 섹션 + 모델별 막대.
...
- **프로젝트 라벨**: 기본은 홈 상대경로(`hsc/rain/foo`). 개인 관습 하드코딩(`hsc/` trim) 제거 → 배포 안전. 줄이고 싶으면 `CC_USAGE_TRIM="hsc,work"`(콤마구분 선두 세그먼트) env로만. 충돌 우려도 이걸로 사라짐.
...
- `.claude-plugin/plugin.json` — `cc-usage` 플러그인 매니페스트.
- `.claude-plugin/marketplace.json` — 로컬 설치용 마켓플레이스 (이 레포 자체가 플러그인, `source: "./"`). 설치: `/plugin marketplace add <레포경로>` → `cc-usage@tool-usage`.
- `hooks/hooks.json` — SessionStart self-install 가드 (`install-daemon.sh ensure` 호출 → 데몬 자동 등록).
- (생성물, `~/.cc-usage/`) `history.csv` — 툴 누적 DB (date,triggered_by,project,tool,count)
- (생성물, `~/.cc-usage/`) `tokens.csv` — 토큰 누적 DB (date,triggered_by,project,model,input,output,cache_read,cw5m,cw1h)
- (생성물, `~/.cc-usage/`) `injects.csv` — 자동주입 누적 DB (date,triggered_by,project,source,count)
- (생성물, `~/.cc-usage/`) `dashboard.html` — 데이터 인라인 self-contained 대시보드
- (생성물, `~/.cc-usage/`) `scheduler.log` — 자동 수집 데몬 실행 로그
